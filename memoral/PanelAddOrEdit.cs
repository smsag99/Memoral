...
namespace memoral
{
    public partial class addProduct : UserControl
    {
        ...
        {
            e.ToPersianInt();               //just persian decimal
        }
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (BaseValidator.IsFormValid(this.components))                 //validatore
            {
                using (DataBaseClass db = new DataBaseClass())
                {
                    if (btnSave.Text.CompareTo("افزودن محصول") == 0)
                    {
                        try
                        {
                            DataLayer.Product p = new DataLayer.Product()                   //adding a new product
                            {
                                ...
                                image = unit_convertor.ImageToByteArray(pic_product.BackgroundImage)
                            };
                    ...
                    else
                    {
                        try
                        {
                            DataLayer.Product p = new DataLayer.Product();                      //editing a product
                            p.ID = productid;
                            ...
                            p.image = unit_convertor.ImageToByteArray(pic_product.BackgroundImage);
        ...
        private void btnChooseImage_click(object sender, EventArgs e)
        {
            try
            {
                this.openFileDialog1.ShowDialog(this);                  //choose image from explore
                string strFn = this.openFileDialog1.FileName;
                this.pic_product.BackgroundImage = Image.FromFile(strFn);
            }
            catch
            {
                RtlMessageBox.Show("عملیات با خطا مواجه شد");
            }
        }
        ...
        private void btnRemove_Click(object sender, EventArgs e)    //remove the product
        {
            if (RtlMessageBox.Show("آیا حذف کردن این محصول اطمینان دارید؟", "هشدار", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                using (DataBaseClass db = new DataBaseClass())
                {
                    DataLayer.Product p = new DataLayer.Product();
                    p.ID = productid;
                    db.ProductRepository.DeleteProduct(p);
                    db.ProductRepository.save();
                }
                RtlMessageBox.Show("محصول شما با موفقیت حذف شد.");
                btnRemoveProduct.Visible = false;
                btnSave.TextAlign = ContentAlignment.MiddleCenter;
                btnSave.Text = "افزودن محصول";
                inputDetails.Text = "";
                inputNewProductFrequery.Text = "";
                inputNewProductName.Text = "";
                inputNewProductPrice.Text = "";
